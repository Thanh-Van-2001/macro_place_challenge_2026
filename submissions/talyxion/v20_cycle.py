...
class HybridV20(_BASE):
    """Multi-cycle GD <-> CD.

    Alternate the v5 analytical placer (gradient descent) and the fast-proxy
    coordinate descent. Each cycle re-runs v5 GD seeded from the CD-refined
    placement, so the coordinated gradient moves can escape the coordinate-
    wise local optimum that plain CD gets stuck in, and CD then re-refines.
    """

    def __init__(self):
        super().__init__()
        self._n_runs = int(os.environ.get("HP20_N", "4"))
        self._cd_budget = float(os.environ.get("HP20_CD_SEC", "180"))
        self._cycles = int(os.environ.get("HP20_CYCLES", "2"))
        self._cycle_iters = int(os.environ.get("HP20_CYCLE_ITERS", "200"))
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        if refined_cost < best_cost:
            best_pos, best_cost = refined, refined_cost
        sys.stderr.write(f"[v20] cycle0 -> {best_cost:.5f}\n")
        sys.stderr.flush()

        # ── Cycles 1..C: re-run v5 GD seeded from the best placement ──
        # Gentle "refinement GD": keep the overlap penalty high and constant
        # (so the already-legal placement is not collapsed) and use a small
        # learning rate + few iterations.
        saved_positions = benchmark.macro_positions
        saved_iters = self.iters
        saved_ov_start = self.ov_start
        saved_ov_end = self.ov_end
        saved_lr = self.lr_frac
        for cyc in range(1, self._cycles + 1):
            try:
                benchmark.macro_positions = torch.tensor(best_pos, dtype=torch.float32)
                self.iters = self._cycle_iters
                self.ov_start = saved_ov_end       # constant high overlap penalty
                self.ov_end = saved_ov_end
                self.lr_frac = saved_lr * 0.4      # gentle steps
                gd = super().place(benchmark)
                gd_pos = gd.cpu().numpy().astype(np.float64)
            finally:
                benchmark.macro_positions = saved_positions
                self.iters = saved_iters
                self.ov_start = saved_ov_start
                self.ov_end = saved_ov_end
                self.lr_frac = saved_lr
            gd_cost = _real_proxy(plc, benchmark, gd_pos)
            cyc_pos, cyc_cost = cd_refine(fe, plc, benchmark, gd_pos, self._cd_budget)
            if cyc_cost <= gd_cost:
                cand, cand_cost = cyc_pos, cyc_cost
            else:
                cand, cand_cost = gd_pos, gd_cost
            if cand_cost < best_cost:
                best_pos, best_cost = cand, cand_cost
            sys.stderr.write(f"[v20] cycle{cyc}: gd={gd_cost:.5f} "
                             f"cd={cyc_cost:.5f} best={best_cost:.5f}\n")
            sys.stderr.flush()
